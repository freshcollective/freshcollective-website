...
def verify_svix_signature(
    ...
    *,
    ...
    """Verify a Svix-format signature (used by Resend).
    ...
    Returns True only when a signature matches AND the timestamp is
...
def _persist_and_process(
    ...
    *,
    ...
    raw_payload_for_audit: Any,
) -> ReceiverOutcome:
    """Persist each parsed event to the ledger and run the mapping.
    If ``events`` is empty (e.g. signature failed), still record a
    single audit row so tampering attempts are visible.
    """
    ...
    def _persist_row(event: WebhookEvent | None, override_error: str | None = None) -> str | None:
        row = CommunicationWebhookEvent(
            id=_new_webhook_event_id(),
            provider_key=provider_key,
            provider_event_id=event.provider_event_id if event else None,
            event_type=event.event_type if event else "signature_verification_failed",
            provider_message_id=event.provider_message_id if event else None,
            signature_verified=signature_verified,
            raw_payload=dict(event.raw) if event else _safe_dict(raw_payload_for_audit),
            process_error=override_error,
        )
    ...
    if not signature_verified:
        _persist_row(None, override_error="signature_verification_failed")
        db.commit()
        return outcome

    if not events:
        # Nothing to map, but audit that we received an empty parse.
        rid = _persist_row(None, override_error="empty_parse")
        if rid:
            outcome.persisted_ids.append(rid)
        db.commit()
        return outcome

    for event in events:
        from app.comms.models import CommunicationWebhookEvent as _CWE
        row = _CWE(
            id=_new_webhook_event_id(),
            provider_key=provider_key,
...
def _safe_dict(raw: Any) -> dict:
    ...
    if isinstance(raw, (bytes, bytearray)):
        try:
            ...
            return {"_raw_text": raw[:1000].decode("utf-8", errors="replace")}
    if isinstance(raw, str):
        try:
            return json.loads(raw)
        except Exception:
            return {"_raw_text": raw[:1000]}
    return {"_raw_repr": repr(raw)[:1000]}
...
def _apply_mapping(
    ...
    *,
    ...
    # Suppression side-effect — hard bounces, complaints,
    # unsubscribes. Applied even when the delivery row is missing
    # (address-only signals still protect future sends).
    if event.recipient_address:
        if et == "bounced" and event.bounce_class == "hard":
            ...
            record_suppression(
                db, address_type="email",
                address=event.recipient_address,
                reason="unsubscribed", source_provider=provider_key,
            )

    # State advance requires a delivery match.
    if not event.provider_message_id:
        # Nothing more to do — suppression handled above (if applicable),
        # opened/clicked/sent are audit-only.
        return
    ...
    if et not in ("delivered", "bounced", "complained"):
        # opened, clicked, sent, unsubscribed, other — audit only.
        return

    # Update delivery terminal outcome
    ...
    # Advance intent state via the M4 state machine.
    ...
    try:
        ...
        # different state). Recorded but not re-raised.
        raise _MappingError(
...
def receive(
    ...
    *,
    ...
    mapping in one call. Returns an outcome for the response body.
    """
    ...
    verified = provider.verify_webhook(headers, raw_body)
    events = provider.parse_webhook(headers, raw_body) if verified else []
    return _persist_and_process(
        db,
        provider_key=provider_key,
        signature_verified=verified,
        events=events,
        raw_payload_for_audit=raw_body,
    )
