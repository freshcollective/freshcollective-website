...
# ---------------------------------------------------------------------------
#
# One entry per real branch in a template's ``render``. These were read
# off the templates rather than guessed: ``added_by_creator`` in
# ``gatherings.py``, ``is_fresh_creator`` in ``account.py``,
# ``was_ticketed``, ``payment_mode``, ``was_suspended`` and
# ``is_full_refund``. A branch that only reflects whether a value
# happens to be present — a missing start time, an empty collective
# name, a one-item schedule — is data shape rather than a product state
# and gets no control.
#
# Each default matches the standing sample context, so a preview with
# no selection renders exactly what it rendered before variants existed.

V_BOOKING_SOURCE = PreviewVariant(
    variant_id="booking_source",
    label="Booking source",
    help_text="Who put the member in the gathering.",
    options=(
        PreviewVariantOption(
            "self_booked", "Member booked",
            {"added_by_creator": False}, is_default=True,
        ),
        PreviewVariantOption(
            "added_by_creator", "Added by creator",
            {"added_by_creator": True},
        ),
    ),
)

V_CREATOR_STATE = PreviewVariant(
    variant_id="creator_state",
    label="Creator state",
    help_text="Whether they have built a Collective yet.",
    options=(
        PreviewVariantOption(
            "returning", "Existing creator",
            {"is_fresh_creator": False}, is_default=True,
        ),
        PreviewVariantOption(
            "fresh", "New creator",
            {"is_fresh_creator": True},
        ),
    ),
)

V_TICKETING = PreviewVariant(
    variant_id="ticketing",
    label="Ticketing",
    help_text="A paid booking gets one extra paragraph about refunds.",
    options=(
        PreviewVariantOption(
            "paid", "Paid ticket",
            {"was_ticketed": True}, is_default=True,
        ),
        PreviewVariantOption(
            "free", "Free gathering",
            {"was_ticketed": False},
...
V_PAYMENT_MODE = PreviewVariant(
    variant_id="payment_mode",
    label="Payment mode",
    help_text="A plan's first payment reads differently from a single one.",
    options=(
        PreviewVariantOption(
            "plan", "Payment plan",
            {"payment_mode": "plan"}, is_default=True,
        ),
        PreviewVariantOption(
            "single", "Paid in full",
            {"payment_mode": "single"},
...
V_ACCESS_AT_RECOVERY = PreviewVariant(
    variant_id="access_state",
    label="Access when the payment landed",
    help_text="Whether the member had already lost access.",
    options=(
        PreviewVariantOption(
            "still_active", "Access stayed active",
            {"was_suspended": False}, is_default=True,
        ),
        PreviewVariantOption(
            "was_paused", "Access had paused",
            {"was_suspended": True},
        ),
    ),
)

V_REFUND_SCOPE = PreviewVariant(
    variant_id="refund_scope",
    label="Refund scope",
    options=(
        PreviewVariantOption(
            "partial", "Partial refund",
            {"is_full_refund": False}, is_default=True,
        ),
        PreviewVariantOption(
            "full", "Full refund",
            {"is_full_refund": True},
...
declare(TemplateDeclaration(
    ...
    preview_variants=(V_CREATOR_STATE,),
))
...
declare(TemplateDeclaration(
    ...
    preview_variants=(V_BOOKING_SOURCE,),
))
...
declare(TemplateDeclaration(
    ...
    preview_variants=(V_PAYMENT_MODE,),
))
...
_system(
    ...
    # Read-only, but it still sends two materially different emails. A
    # preview that showed only one would misrepresent the template.
    variants=(V_ACCESS_AT_RECOVERY,),
)
_system(
